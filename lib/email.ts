...
export async function sendEmail({ to, subject, html }: SendEmailParams): Promise<EmailResult> {
  try {
    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM || "IncentiveIO <noreply@incentiveio.com>",
      to,
      subject,
      html,
    });
...
export async function sendNotificationEmail(to: string, title: string, message: string): Promise<EmailResult> {
  return sendEmail({
    to,
    subject: `[IncentiveIO] ${title}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0ea5e9;">IncentiveIO</h2>
        <h3>${title}</h3>
        <p>${message}</p>
        <p style="color: #666; font-size: 12px;">This is an automated notification from IncentiveIO.</p>
      </div>
    `,
  });
}
...
export async function sendWelcomeEmail(to: string, name: string): Promise<EmailResult> {
  return sendEmail({
    to,
    subject: "Welcome to IncentiveIO",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0ea5e9;">Welcome to IncentiveIO!</h2>
        <p>Hi ${name},</p>
        ...
        <hr />
        <p style="color: #666; font-size: 12px;">This is an automated message from IncentiveIO.</p>
      </div>
    `,
  });
}
...
export async function sendPasswordResetEmail(to: string, name: string, resetUrl: string): Promise<EmailResult> {
  return sendEmail({
    to,
    subject: "Reset Your IncentiveIO Password",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0ea5e9;">IncentiveIO</h2>
        <p>Hi ${name},</p>
